...
public class Servlet_Venta extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(accion.equals("MostrarDetalle2")){
                this.MostrarDetalleVenta2(request, response); 
            }
             if(accion.equals("MostrarVentasMes")){
                this.MostrarVentasMes(request, response); 
            }
            ...
            if(accion.equals("entregar")){
                this.EntregarPelicula(request, response); 
            }
        }
    }
    ...
    private void RegistrarVenta(HttpServletRequest request, HttpServletResponse response)
        ...
        v.setModoPago(request.getParameter("optradio"));
    ...
    private void MostrarDetalleVenta2(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        PrintWriter out = response.getWriter();
        ...
        response.sendRedirect("MostrarDetalleVenta2.jsp"); 
    }
    
      private void MostrarVentasMes(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        PrintWriter out = response.getWriter();

        request.getSession().setAttribute("Fecha", request.getParameter("Fecha"));
        
        response.sendRedirect("MostrarVentasMes.jsp"); 
    }
    ...
    private void EntregarPelicula(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        PrintWriter out = response.getWriter();

        DetalleVenta dv = new DetalleVenta();
        dv.setCodigoVenta(request.getParameter("codigoV"));
        dv.setCodigoPelicula(request.getParameter("codigoP"));
        boolean resp = DetalleVenta_DB.entregarPelicula(dv);
       
        
        if(resp){
            response.sendRedirect("mensaje.jsp?mens='Se ha Entregado la Pelicula "+ dv.getCodigoPelicula() +"'"); 
        }else{
            response.sendRedirect("mensaje.jsp?mens='Error al Entregar la Pelicula " +dv.getCodigoPelicula()+ "'"); 
        }
    }
